	...
	getBrew : (accessType, stubOnly = false)=>{
		...
		return async (req, res, next)=>{
			...
			if(accessType === 'edit' && (authorsExist && !(isAuthor || isInvited))) {
				...
				if(req.account){
					throw { ...accessError, message: 'User is not an Author', HBErrorCode: '03', authors: stub.authors, brewTitle: stub.title, shareId: stub.shareId, published: stub.published };
				}
				throw { ...accessError, message: 'User is not logged in', HBErrorCode: '04', authors: stub.authors, brewTitle: stub.title, shareId: stub.shareId , published: stub.published};
			}
