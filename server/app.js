...
const corsOptions = {
	origin: (origin, callback) => {
		...
		const herokuRegex =
			/^https:\/\/(?:homebrewery-pr-\d+\.herokuapp\.com|naturalcrit-pr-\d+\.herokuapp\.com)$/; // Matches any Heroku app

		if (
			!origin ||
			allowedOrigins.includes(origin) ||
			herokuRegex.test(origin)
		) {
			callback(null, true);
		} else {
			...
			callback(
				new Error(
					"Not allowed by CORS, if you think this is an error, please contact us",
				),
			);
		}
	},
...
const welcomeTextLegacy = fs.readFileSync(
	"client/homebrew/pages/homePage/welcome_msg_legacy.md",
	"utf8",
);
const migrateText = fs.readFileSync(
...
const defaultMetaTags = {
	...
	description:
		"A NaturalCrit Tool for creating authentic Homebrews using Markdown.",
...
app.get("/migrate", (req, res, next) => {
	(req.brew = {
		...
		(req.ogMeta = {
			...
			description:
				"A brief guide to converting Legacy documents to the v3 renderer.",
		});
...
app.get("/user/:username", async (req, res, next) => {
	...
	let brews = await HomebrewModel.getByUser(
		req.params.username,
		ownAccount,
		fields,
	).catch((err) => {
	...
	if (ownAccount && req?.account?.googleId) {
		...
		let googleBrews = await GoogleActions.listGoogleBrews(auth).catch((err) => {
			console.error(err);
		});
		...
		if (googleBrews && googleBrews.length > 0) {
			...
			googleBrews = googleBrews.map((brew) => ({
				...brew,
...
app.put("/api/user/rename", async (req, res) => {
	...
	if (!username || !newUsername)
		return res
			.status(400)
			.json({ error: "Username and newUsername are required." });
	if (!ownAccount)
		return res
			.status(403)
			.json({ error: "Must be logged in to change your username" });
	try {
		...
		return res.json({
			success: true,
			message: `Brews for ${username} renamed to ${newUsername}.`,
		});
	} catch (error) {
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!brew.authors.includes(req.account?.username)) {
			if (req.params.id.length > 12 && !brew._id) {
				...
				await GoogleActions.increaseView(
					googleId,
					shareId,
					"share",
					brew,
		...
		brew.authors.includes(req.account?.username)
			? sanitizeBrew(req.brew, "shareAuthor")
			: sanitizeBrew(req.brew, "share");
		splitTextStyleAndMetadata(req.brew);
		return next();
	}),
	...
	asyncHandler(async (req, res, next) => {
		...
		if (req.account) {
			if (req.account.googleId) {
				...
				googleCount = await GoogleActions.listGoogleBrews(auth).catch((err) => {
					console.error(err);
				});
			}
			...
			const mongoCount = await HomebrewModel.countDocuments(query).catch(
				(err) => {
					mongoCount = 0;
					console.log(err);
				},
			...
			data.accountDetails = {
				...
				authCheck: Boolean(
					req.account.googleId && auth?.credentials.access_token,
				),
...
if (isLocalEnvironment) {
	...
	app.post("/local/login", (req, res) => {
		...
		const payload = jwt.encode(
			{ username: username, issued: new Date() },
			config.get("secret"),
		);
...
app.use(
	"/staticImages",
	express.static(
		config.get("hb_images") && fs.existsSync(config.get("hb_images"))
			? config.get("hb_images")
			: "staticImages",
	),
);
app.use(
	"/staticFonts",
	express.static(
		config.get("hb_fonts") && fs.existsSync(config.get("hb_fonts"))
			? config.get("hb_fonts")
			: "staticFonts",
	),
);
...
const renderPage = async (req, res) => {
	...
	const page = await templateFn("homebrew", title, props).catch((err) => {
		console.log(err);
	});
	return page;
};
...
app.use((req, res) => {
	if (!res.headersSent) {
		console.error(
			"Headers have not been sent, responding with a server error.",
			req.url,
		);
		res
			.status(500)
			.send(
				"An error occurred and the server did not send a response. The error has been logged, please note the time this occurred and report this issue.",
			);
	}
});
//^=====--------------------------------------=====^//
